const Task = ({ taskName, Estimate, SpentTime, Assignee, Priority, status }) => {
    return (
        <div className="py-6 px-7.5 bg-white rounded-3xl flex items-center justify-between">
            <div>
                <p className="text-gray-400 text-sm mb-1.5">Task Name</p>
                <p className="text-darklight">{taskName}</p>
            </div>
            <div className="flex items-center gap-11 w-1/2 justify-between">
                <div>
                    <p className="text-gray-400 text-sm mb-1.5">Estimate</p>
                    <p>{Estimate}</p>
                </div>
                <div>
                    <p className="text-gray-400 text-sm mb-1.5">Spent Time</p>
                    <p>{SpentTime}</p>
                </div>
                <div>
                    <p className="text-gray-400 text-sm mb-1.5">Assignee</p>
                    <img src={Assignee} className="w-6 h-6 rounded-full" />
                </div>
                <div>
                    <p className="text-gray-400 text-sm mb-1.5">Priority</p>
                    {
                        Priority === "Medium" ?
                ...
                <div className="flex w-1/4 items-center justify-between">
                    <p className={`${status == "Done" ? "text-green bg-green-100 px-4 py-1.5 rounded-lg font-bold" : status == "In Progress" ? "text-primary bg-primary-100 px-4 py-1.5 rounded-lg font-bold" : "text-gray-500 bg-gray-200 px-4 py-1.5 rounded-lg font-bold  "}`}>{status}</p>
                    <div className={`w-6 h-6 rounded-full ${status == "Done" ? "border-2 border-primary" : status == "In Progress" ? "border-2 border-primary border-r-gray-400" : "border-2 border-gray-400"}`}></div>
                </div>
            </div>
